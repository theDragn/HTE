...
public class drgArcEmitterMediumWeapon implements OnFireEffectPlugin, EveryFrameWeaponEffectPlugin
{

    // this weapon can only produce one secondary arc

    private static final float CHAIN_CHANCE = 0.33f;
    private static final float CHAIN_RANGE = 200f;
    private static final float BASE_RANGE = 500f; // unmodified weapon range, used for figuring out the proportional increase to chain range
    ...
    // gets list of all hostile targets within range of a point, ignoring angle
        ...
        if (!targetList.isEmpty())
        {
            ...
            if (rand.nextFloat() <= CHAIN_CHANCE)
            {
                ...
                if (!targetList.isEmpty())
                {
                    ...
                    engine.spawnEmpArc(weapon.getShip(), target.getLocation(), target, nextTarget,
                            weapon.getDamageType(), arcDamage, empDamage, 10000f, "tachyon_lance_emp_arc_impact",
                            10f, CORE_COLOR, FRINGE_COLOR);
                }
            }
        }
    }
