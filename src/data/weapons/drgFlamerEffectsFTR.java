...
public class drgFlamerEffectsFTR extends BaseCombatLayeredRenderingPlugin implements OnFireEffectPlugin,
        OnHitEffectPlugin,
        EveryFrameWeaponEffectPlugin
{
    ...
    protected List<drgFlamerEffectsFTR> trails;


    public drgFlamerEffectsFTR()
    {
    }
    ...
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon)
    {
        ...
        if (trails == null || drgModPlugin.LOW_GRAPHICS_MODE)
            return;

        Iterator<drgFlamerEffectsFTR> iter = trails.iterator();
        while (iter.hasNext())
        {
        ...
        if (weapon.getShip() != null)
        {
            ...
            for (drgFlamerEffectsFTR curr : trails)
            {
                if (curr.proj != null)
                {
            ...
            if (weight > 0.1f)
            {
                ...
                Global.getSoundPlayer().playLoop("drg_flamer_loop", ship.getLocation(), 1f, volume, com, ship.getVelocity());
            }
        }
        ...
        {
            for (drgFlamerEffectsFTR trail : trails)
            {
...
//								   rampUp, dur, inverted);
    ...
    public void onFire(DamagingProjectileAPI projectile, WeaponAPI weapon, CombatEngineAPI engine)
    {
        ...
        drgFlamerEffectsFTR trail = new drgFlamerEffectsFTR(projectile, prev);
        ...
        if (trails == null)
        {
            trails = new ArrayList<drgFlamerEffectsFTR>();
        }
        trails.add(0, trail);
    }
...
//			offset.y += vel.y * 1f;
    ...
    public drgFlamerEffectsFTR(DamagingProjectileAPI proj, DamagingProjectileAPI prev)
    {
        this.proj = proj;
        this.prev = prev;
